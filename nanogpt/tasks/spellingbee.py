...
from .common import download_file_with_lock
...
class SpellingBee(Task):
    ...
    def __init__(self, size=1000, split="train", base_dir=None, **kwargs):
        super().__init__(**kwargs)
        assert split in ["train", "test"], "SpellingBee split must be train|test"
        self.size = size
        self.split = split
        filename = WORD_LIST_URL.split("/")[-1]
        word_list_path = download_file_with_lock(WORD_LIST_URL, filename, base_dir=base_dir)
